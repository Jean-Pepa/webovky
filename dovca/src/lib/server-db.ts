import { getRedis, DB_KEY, REV_KEY } from "./redis";
...
// Tím se zabrání ztrátě zápisů při souběhu (např. celá parta klika naráz).
const CAS_SCRIPT = `
...
// Načte DB i s revizí. Pokud je úložiště prázdné, nasází výchozí (demo) stav.
...
// Vrací novou DB; null = backend není nastavený nebo se zápis nepodařil.
export async function applyActionAtomic(action: Action): Promise<DB | null> {
  ...
  for (let attempt = 0; attempt < 6; attempt++) {
    ...
    // jiná revize mezitím → načti znovu a přehraj akci na čerstvém stavu
  }
  return null;
}
