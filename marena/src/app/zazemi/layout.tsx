...
export default function ZazemiLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 pb-[calc(6.5rem+env(safe-area-inset-bottom))] md:pb-6">{children}</main>
      ...
      {sheet &&
        (() => {
          ...
          if (!g) return null;
          return (
            <div className="fixed inset-x-0 bottom-[calc(5.75rem+env(safe-area-inset-bottom))] z-40 mx-3 rounded-2xl border border-ink/10 bg-surface p-3 shadow-2xl md:hidden">
              {g.sections.map((s) => (
      ...
      <nav className="fixed inset-x-3 bottom-[calc(0.75rem+env(safe-area-inset-bottom))] z-40 rounded-[28px] border border-ink/10 bg-paper/95 shadow-lg backdrop-blur md:hidden">
