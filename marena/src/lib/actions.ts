...
export type Action =
  ...
  | { type: "addInvite"; yearId: string; category: string; name: string; link?: string; availability?: string; price?: string; priority?: number; addedBy?: string }
  | {
      type: "updateInvite";
      yearId: string;
      inviteId: string;
      // Prázdný řetězec / null = „smazat" (přežije JSON.stringify; undefined by se zahodilo).
      patch: Partial<Pick<Invite, "category" | "name" | "contacted" | "interest" | "cancelled" | "contactedBy" | "interestBy" | "cancelledBy">> & {
        link?: string;
...
export function applyAction(db: DB, a: Action): DB {
  switch (a.type) {
    ...
    // Správce: úprava uložené (uzavřené) kasy — název, ranní vklad, večerní stav.
    // Přepočítá navázaný zápis rozdílu ve financích (přebytek / manko): když vyjde 0,
    // zápis se odebere; když dřív nebyl a teď je, vytvoří se. Markovaná hotovost
    // (alreadyRecorded) se nemění — ta je z prodejů dne.
    case "updateCashbox":
      return mapYear(db, a.yearId, (y) => {
        const box = (y.cashboxes ?? []).find((c) => c.id === a.cashboxId);
        if (!box) return y;
        ...
        if (box.closedAt && closing != null) {
          ...
          const trzba = closing - opening - already;
      ...
      return mapYear(db, a.yearId, (y) => ({
        ...
        invites: [
          ...(y.invites ?? []),
          {
            ...
            contacted: false,
            interest: "nevim",
            addedBy: a.addedBy?.trim() || undefined,
            createdAt: now(),
          },
        ],
      }));
